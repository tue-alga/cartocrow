...
#include "cartocrow/necklace_map/detail/check_feasible_exact.h"
#include "cartocrow/necklace_map/detail/check_feasible_heuristic.h"
...
bool CheckFeasible::ProcessContainer(const size_t first_slice_index,
	...
	for (ptrdiff_t value_index = num_slices - 1;
		...
		if (value.angle_rad + kEpsilon < value_slice.coverage.from() ||
		    (value.angle_rad < value_slice.coverage.from() + kEpsilon &&
