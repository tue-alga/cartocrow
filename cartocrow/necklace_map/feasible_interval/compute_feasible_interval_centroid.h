...
class ComputeFeasibleCentroidInterval : public ComputeFeasibleInterval {
  public:
	CircularRange operator()(const PolygonSet<Inexact>& shape,
	                         const Necklace& necklace) const override;
	ComputeFeasibleCentroidInterval(const Parameters& parameters);

  private:
	friend ComputeFeasibleInterval;
	Number<Inexact> half_length_rad_;
}; // class ComputeFeasibleCentroidInterval

} // namespace cartocrow::necklace_map

#endif //CARTOCROW_NECKLACE_MAP_COMPUTE_FEASIBLE_CENTROID_INTERVAL_H
