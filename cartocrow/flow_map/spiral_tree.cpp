...
std::optional<SpiralTree::Wavefront::iterator> SpiralTree::handleJoinEvent(const Event& event,
	...
	m_nodes.push_back(event.m_node);
...
SpiralTree::Wavefront::iterator SpiralTree::handleLeafEvent(Event& event, Wavefront& wavefront) {
	...
	if (!wavefront.empty()) {
		...
		if (isReachable(event.m_relative_position, node_circ->second.m_relative_position) ||
		    isReachable(event.m_relative_position, (--node_circ)->second.m_relative_position)) {
...
void SpiralTree::computeUnobstructed() {
	...
	for (std::shared_ptr<Node>& node : m_nodes) {
		...
		const PolarPoint relative_position(node->m_place->position, m_root_translation);
...
void SpiralTree::insertJoinEvent(const Event& first, const Event& second, EventQueue& events) {
	const Spiral spiral_left(first.m_relative_position, -m_restricting_angle);
	const Spiral spiral_right(second.m_relative_position, m_restricting_angle);
...
bool EmbeddingHalfedge::IsValid() const {
	...
	return (bool) opposite && this == opposite->opposite.get() && (bool) face &&
	       (this == face->edge_cw.get() || this == face->edge_ccw.get());
}
...
EmbeddingFace::FacePtr EmbeddingFace::ConstructBetweenEdges(const HalfedgePtr& edge_cw,
                                                            const HalfedgePtr& edge_ccw) {
...
bool EmbeddingFace::IsValid() const {
	return (bool) edge_cw && this == edge_cw->face.get() && (bool) edge_ccw &&
	       this == edge_ccw->face.get();
}
...
struct VertexEvent : public NodeElement {
	...
	VertexEvent(const PolarPoint& relative_position, const EdgeElementPtr& edge_prev,
...
bool EmbeddingNode::IsValid() const {
	return (bool) edge && this == edge->node.get() && (bool) event && this == event->embedding.get();
}
...
template <class T_> struct EdgeElement_ : public EdgeElement {
...
struct FaceElement {
	...
	const NodeElementPtr& Vanishing() const {
		return vanishing_;
	}
...
struct OpenInterval : public FaceElement {
	...
	bool IsFree() const {
		return tag == nullptr || tag->parent != nullptr;
	}
...
enum SweepDirection { kInward, kOutward };

template <SweepDirection D_> struct SweepCompareEvents;

template <> struct SweepCompareEvents<SweepDirection::kInward> { using Type = CompareEvents; };

template <> struct SweepCompareEvents<SweepDirection::kOutward> {
	using Type = CompareEventsReverse;
};

template <SweepDirection D_> class SweepCircle {
  public:
	...
	SweepDirection Direction() const {
		return D_;
	}
...
template <> SweepCircle<SweepDirection::kInward>::SweepCircle(const Node::Ptr& root) : R(-1) {
	EmbeddingHalfedge::HalfedgePtr halfedge = EmbeddingHalfedge::ConstructWithOpposite();

	EmbeddingHalfedge::EdgeElementPtr x_axis =
	    std::make_shared<EdgeHalfline>(PolarPoint(1, 0)); // Relevant: edge geometry.
	halfedge->SetSegment(x_axis);

	EmbeddingFace::FacePtr face = EmbeddingFace::ConstructBetweenEdges(halfedge, halfedge->opposite);

	EmbeddingFace::FaceElementPtr interval =
	    std::make_shared<OpenInterval>(); // Relevant: interval is open.
...
template <> SweepCircle<SweepDirection::kOutward>::SweepCircle(const Node::Ptr& root) : R(0) {
	EmbeddingHalfedge::HalfedgePtr halfedge = EmbeddingHalfedge::ConstructWithOpposite();
	...
	EmbeddingNode::NodeElementPtr root_event =
	    std::make_shared<NodeEvent>(PolarPoint(0, 0), root); // Relevant: node position.
...
class SweepCircleOld {
  public:
	...
	void MergeSimilarNeighbors(IntervalSet::iterator interval_iter,
	                           IntervalSet::iterator neighbor_iter_cw,
	                           IntervalSet::iterator neighbor_iter_ccw,
	                           std::vector<Event::Ptr>& add_events) {
	...
	void RemoveIntervalFollowEdgeCw(IntervalSet::iterator interval_iter,
	                                IntervalSet::iterator neighbor_iter_cw,
	                                IntervalSet::iterator neighbor_iter_ccw,
	                                std::vector<Event::Ptr>& add_events) {
	...
	void RemoveIntervalFollowEdgeCcw(IntervalSet::iterator interval_iter,
	                                 IntervalSet::iterator neighbor_iter_cw,
	                                 IntervalSet::iterator neighbor_iter_ccw,
	                                 std::vector<Event::Ptr>& add_events) {
	...
	void HandleVanishing(const EventPtr& event, std::vector<EventPtr>& add_events) {
		...
		if (open_cw == nullptr && open_ccw == nullptr) {
			...
			RemoveIntervalFollowEdgeCw(interval_iter, neighbor_iter_cw, neighbor_iter_ccw,
			                           add_events);
		} else if (open_ccw == nullptr) {
			RemoveIntervalFollowEdgeCcw(interval_iter, neighbor_iter_cw, neighbor_iter_ccw,
			                            add_events);
	...
	IntervalSet::iterator FindInterval(const PolarPoint& position) {
		...
		IntervalSet::iterator ccw_iter =
		    std::upper_bound(intervals.begin(), intervals.end(), dummy, compare);
...
void SpiralTree::ComputeObstructed() {
	...
	using ReverseQueue =
	    std::priority_queue<EventPtr, std::vector<EventPtr>, obst::CompareEventsReverse>;
	...
	for (const Obstacle& obstacle : obstacles_) {
		...
		for (Obstacle::const_iterator vertex_iter = obstacle.begin(); vertex_iter != obstacle.end();
		     prev_iter = vertex_iter++) {
			...
			vertex = std::make_shared<obst::Event>(*vertex_iter, std::make_shared<Node>(),
			                                       edge_prev, nullptr);
