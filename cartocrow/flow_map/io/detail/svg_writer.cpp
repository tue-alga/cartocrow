...
std::string SpiralToPath(const Spiral& spiral, const Vector& offset, const int precision,
                         const PolarPoint& parent) {
	std::stringstream stream;
	stream << std::setprecision(precision);
 ...
 */
SvgWriter::SvgWriter(const std::vector<Region>& context, const std::vector<Region>& obstacles,
                     const FlowTree::Ptr& tree, const WriteOptions::Ptr& options, std::ostream& out)
    : context_(context), obstacles_(obstacles), tree_(tree), out_(out), options_(options) {
	ComputeBoundingBox();
	OpenSvg();
}
	...
	{
		{
			...
			const double multiply_alpha =
			    options_->flow_opacity < 0 ? 1.0 : kDropShadowShade * options_->flow_opacity;
			std::stringstream stream;
...
void SvgWriter::DrawSpiral(const Spiral& spiral, const Vector& offset, const PolarPoint& parent) {
	...
	printer_.PushAttribute(
	    "d", SpiralToPath(spiral, offset, options_->numeric_precision, parent).c_str());
	printer_.PushAttribute("transform", transform_matrix_.c_str());
	printer_.CloseElement(); // path
}
...
void SvgWriter::DrawObstacleVertices(const Polygon& polygon) {
	for (Polygon::Vertex_const_iterator vertex_iter = polygon.vertices_begin();
