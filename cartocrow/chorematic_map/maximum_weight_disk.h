			...
			for (CGAL::Sign side : {CGAL::NEGATIVE, CGAL::POSITIVE}) {
				...
				std::sort(candidates.begin(), candidates.end(),
				          [pi, pj, vij, m](const WeightedPoint& wp1, const WeightedPoint& wp2) {
					          return abs((CGAL::circumcenter(pi.point, pj.point, wp1.point) - m) * vij) <
					                 abs((CGAL::circumcenter(pi.point, pj.point, wp2.point) - m) * vij);
				          });
				...
				for (auto qit = begin; qit != end; ++qit) {
					auto& q = *qit;
					auto sideOfCircle = CGAL::side_of_bounded_circle(pi.point, pj.point, q.point);
				...
				for (auto& cand : candidates) {
					...
					double sqRadius = CGAL::squared_distance(pi.point, CGAL::circumcenter(pi.point, pj.point, cand.point));
				...
				if (bestTotalWeight > overallBestWeight ||
				    bestTotalWeight == overallBestWeight && sqRadiusOfBest < overallSquaredRadius) {
					overallBestWeight = bestTotalWeight;
					overallBestTriple = {pi, pj, bestCandidate};
					overallSquaredRadius = sqRadiusOfBest;
				}
			}
		}
	}
