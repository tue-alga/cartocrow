...
std::string SpiralToPath(const Spiral& spiral, const Vector& offset, const Number& R_max,
                         const Number& R_min, const int precision) {
	std::stringstream stream;
	...
	const Point near =
	    (R_min == 0 ? Point(0, 0) : spiral.Evaluate(spiral.ComputeT(R_min)).to_cartesian()) + offset;
	stream << " L " << near.x() << " " << near.y();
	return stream.str();
}

std::string LineToPath(const PolarLine& line, const Vector& offset, const Number& t_from,
                       const Number& t_to, const int precision) {
	std::stringstream stream;
	stream << std::setprecision(precision);
...
Box Offset(const Box& bounding_box, const Vector& offset) {
	return Box(bounding_box.xmin() + offset.x(), bounding_box.ymin() + offset.y(),
	           bounding_box.xmax() + offset.x(), bounding_box.ymax() + offset.y());
}
...
SvgWriter::SvgWriter(const std::vector<PolarPoint>& points, const std::vector<Spiral>& spirals,
    ...
    : points_(points), spirals_(spirals), spiral_segments_(spiral_segments), lines_(lines),
      line_segments_(line_segments), out_(out), options_(options) {
	ComputeBoundingBox();
	OpenSvg();
}
...
void SvgWriter::DrawSpiral(const Spiral& spiral, const Number& R_max, const Number& R_min /*= 0*/) {
	...
	printer_.PushAttribute("style", style.c_str());
	printer_.PushAttribute(
	    "d", SpiralToPath(spiral, Vector(0, 0), R_max, R_min, options_->numeric_precision).c_str());
	printer_.PushAttribute("transform", transform_matrix_.c_str());
...
void SvgWriter::DrawLine(const PolarLine& line, const Number& t_from, const Number& t_to) {
	...
	printer_.PushAttribute("style", style.c_str());
	printer_.PushAttribute(
	    "d", LineToPath(line, Vector(0, 0), t_from, t_to, options_->numeric_precision).c_str());
	printer_.PushAttribute("transform", transform_matrix_.c_str());
	printer_.CloseElement(); // path
}
