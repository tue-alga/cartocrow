...
Polygon<Exact> close_isoline(const Isoline<K>& isoline, Exact::Iso_rectangle_2& bb) {
	...
	if (!isoline.m_closed) {
		...
		auto source_side = closest_side(source, bb);
		auto target_side = closest_side(target, bb);
		...
		auto source_out = source + source_dir;
		auto target_out = target + target_dir;
...
double symmetric_difference(const Isoline<K>& isoline1, const Isoline<K>& isoline2) {
	...
	std::transform(isoline1.m_points.begin(), isoline1.m_points.end(), std::back_inserter(all_points),
				   [](Point<Inexact> p) { return Point<Exact>(p.x(), p.y()); });
	std::transform(isoline2.m_points.begin(), isoline2.m_points.end(), std::back_inserter(all_points),
				   [](Point<Inexact> p) { return Point<Exact>(p.x(), p.y()); });

	Exact::Iso_rectangle_2 bb = CGAL::bounding_box(all_points.begin(), all_points.end());

	auto p1 = close_isoline(isoline1, bb);
	auto p2 = close_isoline(isoline2, bb);

	if (!isoline1.m_closed && !isoline2.m_closed) {
		auto source1_side = closest_side(
		    Point<Exact>(isoline1.m_points.front().x(), isoline1.m_points.front().y()), bb);
		auto target1_side = closest_side(
		    Point<Exact>(isoline1.m_points.back().x(), isoline1.m_points.back().y()), bb);
		auto source2_side = closest_side(
		    Point<Exact>(isoline2.m_points.front().x(), isoline2.m_points.front().y()), bb);
		auto target2_side = closest_side(
		    Point<Exact>(isoline2.m_points.back().x(), isoline2.m_points.back().y()), bb);
		assert(source1_side == source2_side);
		assert(target1_side == target2_side);
	}
