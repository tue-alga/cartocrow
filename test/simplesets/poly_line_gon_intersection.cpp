...
TEST_CASE("Crashes") {
	...
	ipeRenderer.addPainting([&pgn, &pln](GeometryRenderer& renderer) {
		auto path = renderPath(pgn);
		renderer.draw(path);
	  	renderer.draw(renderPath(pln));
	});
	ipeRenderer.save("/home/steven/Documents/cartocrow/crash.ipe");
}
