/*
Generic reader for SVG geometry files.
 ...
 * @brief An interface for converting a SVG path element to another data type.
 */
 ...
 * @param to the absolute coordinates of the other endpoint of the line segment.
 *
 ...
 *
 * This Bezier curve starts at the previous endpoint. Its control point mirrors the
 * control point of the previous curve in the previous endpoint.
 * @param to the absolute coordinates of the endpoint of the curve.
 */
 ...
 *
 * This Bezier curve starts at the previous endpoint. Its first control point mirrors
 * the last control point of the previous curve in the previous endpoint.
 * @param control_2 the absolute coordinates of the second Bezier control point.
 * @param to the absolute coordinates of the endpoint of the curve.
 */
 ...
 *
 * For a more in-depth description of these curves, please refer to SVG path
 * documentation, e.g. https://developer.mozilla.org/en-US/docs/Web/SVG/Tutorial/Paths
 * @param radius_x the horizontal radius of the reference ellipse.
 * @param radius_y the vertical radius of the reference ellipse.
 * @param rotation_ccw_rad the counterclockwise rotation of the reference ellipse in
 * radians.
 * @param long_arc whether the longer arc on the ellipse should be used.
 * @param sweep_ccw whether the arc should traverse the ellipse in counterclockwise
 ...
 *
 * This line segment is incident to the previous (end)point.
 * @param to the coordinates of the other endpoint of the line segment relative to the
 * previous (end)point.
 *
 * Either coordinate may be std::nan to indicate an axis aligned line segment.
 */
 ...
 *
 * This Bezier curve starts at the previous (end)point.
 * @param control the coordinates of the Bezier control point relative to the previous
 * (end)point.
 * @param to the coordinates of the endpoint of the curve relative to the previous
 * (end)point.
 */
 ...
 *
 * This Bezier curve starts at the previous (end)point.
 * @param control_1 the coordinates of the first Bezier control point relative to the
 * previous (end)point.
 * @param control_2 the coordinates of the second Bezier control point relative to the
 * previous (end)point.
 * @param to the coordinates of the endpoint of the curve relative to the previous
 * (end)point.
 */
 ...
 * @param sweep_ccw whether the arc should traverse the ellipse in counterclockwise
 * direction from the previous point.
 * @param to the coordinates of the endpoint of the arc relative to the previous
 * (end)point.
 */
inline void SvgPathConverter::EllipticalArcTo
(
...
/**@brief Convert a SVG path string to some other data format.
...
bool SvgPathParser::operator()(const std::string& path, SvgPathConverter& converter)
{
  ...
  while (ss)
  {
    try
    {
      ...
      // Note that SVG uses a y-down coordinate system, while Leaflet expects north-up
      // coordinates.
      // Also note that the order of parsing the stream is important, so it cannot be
      // done as part of the converter method parameters (C++ has undefined parameter
      // evaluation order).
      switch (command)
      {
        case kAbsoluteMove:
          converter.MoveTo(pp.getPoint(ss));
          break;
        case kAbsoluteLine:
          converter.LineTo(pp.getPoint(ss));
          break;
        case kAbsoluteHorizontalLine:
          converter.LineTo(Point(pp.getX(ss), 0.0/0.0));
          break;
        case kAbsoluteVerticalLine:
          converter.LineTo(Point(0.0/0.0, pp.getY(ss)));
          break;
        case kAbsoluteQuadraticBezier:
        {
          const Point control = pp.getPoint(ss);
          const Point point = pp.getPoint(ss);
          converter.QuadBezierTo(control, point);
          break;
        }
        case kAbsoluteContinueQuadraticBezier:
          converter.ContinueQuadBezierTo(pp.getPoint(ss));
          break;
        case kAbsoluteCubicBezier:
        {
          const Point control_1 = pp.getPoint(ss);
          const Point control_2 = pp.getPoint(ss);
          const Point point = pp.getPoint(ss);
        ...
        case kAbsoluteClose:
          converter.Close();
        case kRelativeMove:
          converter.MoveTo(pp.getVector(ss));
          break;
        case kRelativeLine:
          converter.LineTo(pp.getVector(ss));
          break;
        case kRelativeHorizontalLine:
          converter.LineTo(Vector(pp.getX(ss), 0.0/0.0));
          break;
        case kRelativeVerticalLine:
          converter.LineTo(Vector(0.0/0.0, pp.getY(ss)));
          break;
        case kRelativeQuadraticBezier:
        {
          const Vector control = pp.getVector(ss);
          const Vector point = pp.getVector(ss);
          converter.QuadBezierTo(control, point);
          break;
        }
        case kRelativeContinueQuadraticBezier:
          converter.ContinueQuadBezierTo(pp.getVector(ss));
          break;
        case kRelativeCubicBezier:
        {
          const Vector control_1 = pp.getVector(ss);
          const Vector control_2 = pp.getVector(ss);
          const Vector point = pp.getVector(ss);
          converter.CubeBezierTo(control_1, control_2, point);
          break;
        }
