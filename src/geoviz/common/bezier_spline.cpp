...
bool BezierSpline::IsClosed() const
{
  return
    IsContinuous() &&
    curves_.front().source() == curves_.back().target();
}
...
bool BezierSpline::ToCircle(Circle& circle, const Number& epsilon /*= 0.01*/) const
{
  ...
  struct SquaredDistance
  {
    SquaredDistance(const Point& kernel) :
      kernel(kernel),
      squared_distance_min(-1),
      squared_distance_max(0)
    {}

    void operator()(const Point& point)
    {
      const Number squared_distance = CGAL::squared_distance(kernel, point);
      squared_distance_min = squared_distance_min < 0 ? squared_distance : std::min(squared_distance_min, squared_distance);
      squared_distance_max = std::max(squared_distance_max, squared_distance);
    }

    inline Number SquaredRadius() const { return (squared_distance_min + squared_distance_max) / 2.0; }
 ...
 * @param target the target point of the curve.
 */
void BezierSpline::AppendCurve(const Point& source_control, const Point& target_control, const Point& target)
{
  CHECK(!curves_.empty());
...
{
  CurveSet reverse;
  for (CurveSet::reverse_iterator curve_iter = curves_.rbegin(); curve_iter != curves_.rend(); ++curve_iter )
  {
    reverse.emplace_back
    (
      curve_iter->target(),
      curve_iter->target_control(),
      curve_iter->source_control(),
      curve_iter->source()
    );
  }
