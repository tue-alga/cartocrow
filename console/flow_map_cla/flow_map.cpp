...
DEFINE_string(in_geometry_filename, "", "The filename for the map geometry input.");

DEFINE_string(in_data_filename, "", "The filename for the numeric data input.");

DEFINE_string(in_obstacles_filename,
...
DEFINE_double(restricting_angle_rad, 0.43633,
              "Maximum angle between the line connecting the root and any point on a tree arc and "
              "arc's tangent line at that point. Must be in the range (0, pi/2).");

DEFINE_int32(pixel_width, 500, "Output pixel width. Must be strictly positive.");

DEFINE_int32(coordinate_precision, 5,
...
DEFINE_double(obstacle_opacity, -1,
...
DEFINE_double(flow_opacity, 1,
              "Opacity of the flow tree in the output. Must be in the range [0, 1].");

DEFINE_double(node_opacity, 1, "Opacity of the nodes in the output. Must be in the range [0, 1].");

void ValidateFlags(cartocrow::flow_map::Parameters& parameters,
	...
	write_options = Options::Default();
	{
		...
		correct &= CheckAndPrintFlag(FLAGS_NAME_AND_VALUE(coordinate_precision),
		...
		correct &=
		    CheckAndPrintFlag(FLAGS_NAME_AND_VALUE(obstacle_opacity), MakeUpperBoundCheck(1.0));
...
bool ReadData(const std::string& filename, std::vector<cartocrow::flow_map::Place::Ptr>& places,
              size_t& root_index) {
	cartocrow::flow_map::DataReader data_reader;
	return data_reader.ReadFile(filename, FLAGS_in_value_name, places, root_index);
}

void WriteOutput(const std::vector<cartocrow::Region>& context,
...
int main(int argc, char** argv) {
	...
	CHECK(FLAGS_in_obstacles_filename.empty()
	          ? true
	          : ReadGeometry(FLAGS_in_obstacles_filename, obstacles, waypoints))
	    << "Terminating program.";
