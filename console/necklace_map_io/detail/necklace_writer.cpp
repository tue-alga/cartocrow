...
void Split(const std::string& string, const std::string& split, std::string& before,
           std::string& after) {
...
class NecklaceIntervalVisitor : public necklace_map::NecklaceShapeVisitor {
  public:
	...
	void Visit(necklace_map::BezierNecklace& shape) {
		...
		const Number radius =
		    0.9 *
		    std::min(std::min(kernel.x() - bounding_box.xmin(), bounding_box.xmax() - kernel.x()),
		             std::min(kernel.y() - bounding_box.ymin(), bounding_box.ymax() - kernel.y()));

		interval_shape_ =
		    std::make_shared<necklace_map::CircleNecklace>(Circle(kernel, radius * radius));
	}
...
class DrawNecklaceShapeVisitor : public necklace_map::NecklaceShapeVisitor {
  public:
	DrawNecklaceShapeVisitor(const std::string& necklace_style, const std::string& transform_matrix,
	...
	void Visit(necklace_map::CircleNecklace& shape) override {
		...
		if (shape.draw_bounds_cw_rad() == shape.draw_bounds_ccw_rad()) {
			...
			std::string path;
			{
				...
				stream << kAbsoluteArc << " " << radius << " " << radius << " 0 "
				       << (large_arc ? "1" : "0") << " 1 ";
				stream << ccw.x() << " " << ccw.y() << " ";
 ...
 */
SvgWriter::SvgWriter(const std::vector<MapElement::Ptr>& elements,
                     const std::vector<Necklace::Ptr>& necklaces, const Number& scale_factor,
                     const WriteOptions::Ptr& options, std::ostream& out)
    : elements_(elements), necklaces_(necklaces), scale_factor_(scale_factor), out_(out),
      options_(options) {
	ComputeBoundingBox();
		...
		for (const MapElement::Ptr& element : elements_) {
			...
			if (element->bead) {
				...
				if (element->bead->feasible) {
					std::stringstream stream;
					stream << element->bead->feasible->from_rad() << " "
					       << element->bead->feasible->to_rad();
...

	{
		...
		for (const Necklace::Ptr& necklace : necklaces_) {
			for (const Bead::Ptr& bead : necklace->beads) {
				...
				printer_.OpenElement("circle");
				{
					const std::string base_style =
					    ForceStyle(bead->region_style, "fill-opacity:", options_->bead_opacity);
		...
		for (const Necklace::Ptr& necklace : necklaces_) {
			for (const Bead::Ptr& bead : necklace->beads) {
				...
				printer_.OpenElement("path");
				{
					...
					style_interval =
					    ForceStyle(style_interval, "stroke-width:", kLineWidthPx * unit_px_);
				...
				}
				{
					...
					std::stringstream stream;
					stream << std::setprecision(kIntervalNumericPrecision);
					stream << "M " << endpoint_cw.x() << " " << endpoint_cw.y() << " A " << radius
					       << " " << radius << " 0 " << large_arc_flag << " 1 " << endpoint_ccw.x()
					       << " " << endpoint_ccw.y();
		...
		for (const Necklace::Ptr& necklace : necklaces_) {
			for (const Bead::Ptr& bead : necklace->beads) {
				...
				printer_.OpenElement("path");
				{
					...
					style_interval =
					    ForceStyle(style_interval,
					               "stroke-width:", kDebugLineWidthRatio * kLineWidthPx * unit_px_);
					style_interval = ForceStyle(style_interval, "stroke:", color);
					style_interval =
					    ForceStyle(style_interval, "stroke-opacity:", kValidIntervalOpacity);
					printer_.PushAttribute("style", style_interval.c_str());
				}
				{
					...
					std::stringstream stream;
					stream << std::setprecision(kIntervalNumericPrecision);
					stream << "M " << endpoint_cw.x() << " " << endpoint_cw.y() << " L "
					       << interval_shape->kernel().x() << " " << interval_shape->kernel().y()
...
void SvgWriter::DrawRegionAngles() {
...

	{
		...
		for (const MapElement::Ptr& element : elements_) {
			...
			style =
			    ForceStyle(style, "stroke-width:", kDebugLineWidthRatio * kLineWidthPx * unit_px_);
			printer_.PushAttribute("style", style.c_str());

			{
				...
				std::stringstream stream;
				stream << std::setprecision(kIntervalNumericPrecision);
				stream << "M " << kernel.x() << " " << kernel.y() << " L " << endpoint.x() << " "
				       << endpoint.y();
...
void SvgWriter::OpenSvg() {
...

	{
		...
		std::stringstream stream;
		stream << "matrix(" << kTransformScale << ",0,"
		       << "0," << -kTransformScale << "," << -kTransformScale * bounding_box_.xmin() << ","
		       << kTransformScale * bounding_box_.ymax() << ")";
...
void SvgWriter::ComputeBoundingBox() {
	...
	const Number buffer =
	    kBoundingBoxBufferPx * (bounding_box_.xmax() - bounding_box_.xmin()) / options_->pixel_width;
	...
	for (const Necklace::Ptr& necklace : necklaces_) {
		...
		for (const Bead::Ptr& bead : necklace->beads) {
			...
			if (options_->draw_feasible_intervals) {
				...
				const Number radius =
				    interval_shape->ComputeRadius() + kLineWidthPx * unit_px_ * ++count;
				bead_interval_map_[bead] = std::make_shared<CircleNecklace>(
				    Circle(necklace->shape->kernel(), radius * radius));
			} else {
	...
	{
		{
			...
			const double multiply_alpha =
			    options_->bead_opacity < 0 ? 1.0 : kDropShadowShade * options_->bead_opacity;
			std::stringstream stream;
...

	{
		...
		for (const MapElement::Ptr& element : elements_) {
			...
			printer_.PushAttribute("dominant-baseline", "central");
			{
				...
				const Point transformed(kTransformScale * (position.x() - bounding_box_.xmin()),
