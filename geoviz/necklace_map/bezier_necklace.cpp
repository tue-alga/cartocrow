  ...
  kernel_(kernel)
{
  ...
  if
  (
    CGAL::orientation(spline_.curves().begin()->source(), spline_.curves().begin()->source_control(), kernel_) ==
    CGAL::CLOCKWISE
  )
    spline_.Reverse();
...
bool BezierNecklace::IsValid() const
{
  ...
  for (const BezierCurve& curve : spline_.curves())
    valid &=
      curve.source() != curve.source_control() &&
...
Number BezierNecklace::ComputeCoveringRadiusRad(const Range::Ptr& range, const Number& radius) const
{
  ...
  const BezierSpline::CurveSet::const_iterator curve_iter_from = FindCurveContainingAngle(range->from());
  ...
  for (BezierSpline::CurveSet::const_iterator curve_iter = curve_iter_from; curve_iter != curve_iter_to || t <= t_to;)
  {
...
BezierSpline::CurveSet::const_iterator BezierNecklace::FindCurveContainingAngle(const Number& angle_rad) const
{
  BezierSpline::CurveSet::const_iterator curve_iter = std::lower_bound
  (
    spline_.curves().begin(),
    spline_.curves().end(),
    Modulo(angle_rad),
...
bool BezierNecklace::IntersectRay(const Number& angle_rad, const BezierSpline::CurveSet::const_iterator& curve_iter, Point& intersection, Number& t) const
{
