 ...
 */
PolarLine::PolarLine(const PolarPoint& closest) :
  foot_(closest)
{}
...
Number PolarLine::SetFoot(const PolarPoint& point_1, const PolarPoint& point_2)
{
  ...
  const Number x = (point_2.R() * std::sin(point_2.phi()) - point_1.R() * std::sin(point_1.phi())) / c;
  const Number y = -(point_2.R() * std::cos(point_2.phi()) - point_1.R() * std::cos(point_1.phi())) / c;
